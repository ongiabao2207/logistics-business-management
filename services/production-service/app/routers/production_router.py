...
from app.schemas.production_schema import (
    ...
    ProductionDetailsReplace,
    ProductionPeriodCreate,
    ProductionPeriodDetailResponse,
...
def get_production_service(
    ...
    contract_client: ContractClient = Depends(get_contract_client),
) -> ProductionService:
...
def check_overlap(
    ...
    _user: Annotated[CurrentUser, Depends(require_roles("ROLE_OPERATION"))],
...
def create_draft(
    payload: ProductionPeriodCreate,
    current_user: Annotated[CurrentUser, Depends(require_roles("ROLE_OPERATION"))],
    service: ProductionService = Depends(get_production_service),
) -> ProductionPeriodResponse:
...
def list_production_periods(
    current_user: Annotated[CurrentUser, Depends(require_roles("ROLE_OPERATION", "ROLE_ACCOUNTANT"))],
    customer_id: str | None = Query(default=None),
    contract_id: str | None = Query(default=None),
    service: ProductionService = Depends(get_production_service),
) -> list[ProductionPeriodResponse]:
...
def replace_production_details(
    period_id: int,
    payload: ProductionDetailsReplace,
    _user: Annotated[CurrentUser, Depends(require_roles("ROLE_OPERATION"))],
    service: ProductionService = Depends(get_production_service),
) -> ProductionPeriodDetailResponse:
