    ...
    <div className="prod-container">
      ...
      <section className={`prod-stats-grid${canManage ? "" : " accountant-summary"}`} aria-label="Tổng quan kỳ sản lượng">
        {canManage ? (
          ...
          <SummaryCard icon={<Lock size={22} />} tone="locked" label="Kỳ sản lượng đã khóa" value={`${lockedCount} kỳ`} />
        )}
      </section>
      ...
      <section className="table-panel prod-list-panel">
        ...
        {isLoading ? (
          <div className="prod-table-state">Đang tải danh sách kỳ sản lượng...</div>
        ) : filteredPeriods.length === 0 ? (
          <div className="data-state prod-empty-state">
            <AlertCircle size={34} aria-hidden="true" />
            <h3>Không tìm thấy kỳ sản lượng nào</h3>
          ...
          <div className="prod-table-scroll">
            <table className="record-table prod-record-table">
              <thead>
                <tr>
                  <th>Mã kỳ sản lượng</th>
                  <th>Khách hàng</th>
                  <th>Hợp đồng</th>
                  <th>Thời gian áp dụng</th>
                  <th>Trạng thái</th>
                  <th className="prod-actions-column">Thao tác</th>
                </tr>
              </thead>
              <tbody>
                {filteredPeriods.map((period) => (
                  <ProductionRow
                    ...
                    period={period}
                    canManage={canManage}
                    onDetail={setSelectedDetailPeriodId}
                    onLock={setPeriodToLock}
                  />
                ))}
              </tbody>
            </table>
      ...
      <LockConfirmationModal
        ...
        onSuccess={() => {
          fetchPeriods();
          if (selectedDetailPeriodId === periodToLock?.id) setSelectedDetailPeriodId(null);
        }}
      />
    </div>
  );
...
function SummaryCard({ icon, tone, label, value }) {
  return (
    <div className={`prod-stat-card ${tone}`}>
      <div className={`prod-stat-icon ${tone}`}>{icon}</div>
...
function FilterTab({ active, label, count, onClick }) {
  return (
    <button className={`prod-tab${active ? " active" : ""}`} type="button" onClick={onClick}>
...
function ProductionRow({ period, canManage, onDetail, onLock }) {
  ...
  return (
    <tr>
      <td>
        <strong className="prod-period-code">{period.period_name || `SL-${period.id}`}</strong>
        {period.period_name && !period.period_name.startsWith("SL-") && <span className="prod-period-note">{period.period_name}</span>}
      </td>
      ...
      <td className="prod-row-actions">
        <div>
          <button type="button" className="btn-action secondary" onClick={() => onDetail(period.id)}><Eye size={14} /> Chi tiết</button>
          {canManage && !isLocked && <button type="button" className="btn-action lock" onClick={() => onLock(period)}><Lock size={13} /> Khóa kỳ</button>}
        </div>
      </td>
    </tr>
  );
