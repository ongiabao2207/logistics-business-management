...
import { ArrowLeft, CalendarDays, Check, ChevronDown, Eye, PackagePlus, Pencil, Plus, Search, Send, Trash2, X } from "lucide-react";
...
export function PriceListPage() {
  ...
  const [priceDetailMode, setPriceDetailMode] = useState("view");
  ...
  const updatePrice = useMutation(mutation(({ id, payload }) => priceApi.updatePriceList(id, { ...payload, effective_from: toIsoDate(payload.effective_from), effective_to: toIsoDate(payload.effective_to) }), (data) => { refresh(); queryClient.setQueryData(["prices", "price-list", data.id], data); setSelectedPriceId(null); setEditPrice(null); }));
  const deletePrice = useMutation(mutation(priceApi.deletePriceList, () => { refresh(); setSelectedPriceId(null); setDeleteTarget(null); }));
  ...
  function openPrice(item, mode = "view") { setSelectedPriceId(item.id); setPriceDetailMode(mode); setEditPrice({ description: item.description, effective_from: dateText(item.effective_from), effective_to: dateText(item.effective_to), details: item.details.map((detail) => ({ service_id: String(detail.service_id), unit_price: String(detail.unit_price) })) }); }
  ...
  return <section className="workspace">
    ...
    <div className="segmented-tabs"><button className={tab === "prices" ? "is-active" : ""} type="button" onClick={() => setTab("prices")}>Bảng giá ({priceLists.length})</button><button className={tab === "services" ? "is-active" : ""} type="button" onClick={() => setTab("services")}>Danh mục dịch vụ ({services.length})</button></div>
    {tab === "prices" && <PriceListTab rows={rows} query={query} setQuery={setQuery} status={status} setStatus={setStatus} canManage={canManage} canApprove={canApprove} openPrice={openPrice} requestDelete={setDeleteTarget} submitPrice={submitPrice} approvePrice={approvePrice} rejectPrice={rejectPrice} />}
    ...
    {selectedPriceId && <PriceDetailModal query={detailQuery} edit={editPrice} setEdit={setEditPrice} services={services} serviceMap={serviceMap} canManage={canManage} mode={priceDetailMode} update={updatePrice} close={() => setSelectedPriceId(null)} />}
    {deleteTarget && <DeletePriceConfirmation item={deleteTarget} mutation={deletePrice} close={() => { deletePrice.reset(); setDeleteTarget(null); }} />}
  </section>;
}

function PriceListTab({ rows, query, setQuery, status, setStatus, canManage, canApprove, openPrice, requestDelete, submitPrice, approvePrice, rejectPrice }) {
  return <><div className="filter-card identity-filters"><label className="field-wide"><span>Tìm kiếm</span><div className="input-with-icon"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Mã hoặc tên bảng giá..." /></div></label><label><span>Trạng thái</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">Tất cả</option>{Object.entries(statusLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div><div className="table-card"><div className="card-heading"><div><h2>Bảng giá trong database</h2><p>{rows.length} kết quả</p></div></div><div className="table-scroll"><table className="business-table"><thead><tr><th>Mã</th><th>Tên bảng giá</th><th>Số dịch vụ</th><th>Hiệu lực</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{rows.map((item) => { const canEdit = canManage && ["DRAFT", "REJECTED"].includes(item.status); const canDelete = canManage && item.status === "DRAFT"; return <tr key={item.id}><td><strong className="code-text">{item.id}</strong></td><td><strong>{item.description}</strong></td><td>{item.details.length}</td><td>{dateText(item.effective_from)} – {dateText(item.effective_to)}</td><td><Status status={item.status} /></td><td><div className="row-actions"><Action icon={<Eye size={19} />} title="Xem chi tiết" click={() => openPrice(item, "view")} />{canManage && <><Action icon={<Pencil size={18} />} title={canEdit ? "Chỉnh sửa" : "Chỉ bảng giá nháp hoặc bị từ chối mới được sửa"} disabled={!canEdit} click={() => openPrice(item, "edit")} /><Action className="danger" icon={<Trash2 size={18} />} title={canDelete ? "Xóa" : "Chỉ bảng giá nháp mới được xóa"} disabled={!canDelete} click={() => requestDelete(item)} /></>}{canManage && item.status === "DRAFT" && <Action icon={<Send size={18} />} title="Gửi duyệt" click={() => submitPrice.mutate(item.id)} />}{canApprove && item.status === "SUBMITTED" && <><Action className="approve" icon={<Check size={19} />} title="Phê duyệt" click={() => approvePrice.mutate(item.id)} /><Action className="danger" icon={<X size={19} />} title="Từ chối" click={() => rejectPrice.mutate(item.id)} /></>}</div></td></tr>; })}</tbody></table></div></div></>;
}
...
function PriceDetailModal({ query, edit, setEdit, services, serviceMap, canManage, mode, update, close }) {
  const item = query.data;
  const editable = Boolean(item && ["DRAFT", "REJECTED"].includes(item.status));
  const editing = mode === "edit" && canManage && editable;
  const selectedIds = (edit?.details ?? []).map((detail) => String(detail.service_id)).filter(Boolean);
  function updateDetail(index, field, value) { setEdit({ ...edit, details: edit.details.map((detail, detailIndex) => detailIndex === index ? { ...detail, [field]: value } : detail) }); }
  function save() {
    update.mutate({ id: item.id, payload: { ...edit, details: edit.details.map((detail) => ({ service_id: Number(detail.service_id), unit_price: Number(detail.unit_price) })) } });
  }
  return <div className="modal-backdrop" onMouseDown={close}><div className="modal-card modal-wide" onMouseDown={(event) => event.stopPropagation()}><ModalHeader title={editing ? "Chỉnh sửa bảng giá" : "Chi tiết bảng giá"} description={editing ? "Cập nhật thông tin và đơn giá trước khi gửi duyệt" : "Thông tin bảng giá"} close={close} />{query.isLoading || !edit ? <DataState title="Đang tải chi tiết..." /> : item && <><div className="account-detail-summary"><div><small>Mã bảng giá</small><strong>{item.id}</strong></div><Status status={item.status} /></div><div className="form-grid three-columns"><Field label="Tên bảng giá" value={edit.description} change={(value) => setEdit({ ...edit, description: value })} disabled={!editing} /><Field label="Ngày bắt đầu" type="date" value={edit.effective_from} change={(value) => setEdit({ ...edit, effective_from: value })} disabled={!editing} /><Field label="Ngày kết thúc" type="date" value={edit.effective_to} change={(value) => setEdit({ ...edit, effective_to: value })} disabled={!editing} /></div>{editing ? <div className="edit-price-lines"><div className="edit-price-lines-heading"><h3>Chi tiết đơn giá</h3><button className="button outline" type="button" onClick={() => setEdit({ ...edit, details: [...edit.details, { service_id: "", unit_price: "" }] })}><Plus size={16} /> Thêm dịch vụ</button></div><div className="table-scroll"><table className="business-table"><thead><tr><th>Dịch vụ</th><th>Đơn vị</th><th>Đơn giá</th><th></th></tr></thead><tbody>{edit.details.map((detail, index) => <tr key={index}><td><ServiceSelect services={services} value={detail.service_id} selectedIds={selectedIds} onChange={(value) => updateDetail(index, "service_id", value)} /></td><td>{serviceMap[detail.service_id]?.unit ?? "—"}</td><td><input className="price-input" type="number" min="0" value={detail.unit_price} onChange={(event) => updateDetail(index, "unit_price", event.target.value)} required /></td><td><Action className="danger" icon={<Trash2 size={18} />} title="Xóa dòng" disabled={edit.details.length === 1} click={() => setEdit({ ...edit, details: edit.details.filter((_, detailIndex) => detailIndex !== index) })} /></td></tr>)}</tbody></table></div></div> : <div className="detail-lines">{item.details.map((detail) => <div key={detail.id}><span>{serviceMap[detail.service_id]?.name ?? `Dịch vụ #${detail.service_id}`}</span><strong>{Number(detail.unit_price).toLocaleString("vi-VN")} VNĐ</strong></div>)}</div>}<Error mutation={update} />{editing && <div className="modal-actions"><button className="button ghost" type="button" onClick={close}>Hủy</button><button className="button navy" type="button" disabled={update.isPending || !edit.details.every((detail) => detail.service_id && detail.unit_price !== "")} onClick={save}>{update.isPending ? "Đang lưu..." : "Lưu thay đổi"}</button></div>}</>}</div></div>;
}

function DeletePriceConfirmation({ item, mutation, close }) {
  return <div className="modal-backdrop" onMouseDown={close}><div className="modal-card" onMouseDown={(event) => event.stopPropagation()}><ModalHeader title="Xóa bảng giá?" description="Thao tác này không thể hoàn tác" close={close} /><div className="confirm-content"><p>Bạn có chắc muốn xóa bảng giá <strong>{item.id}</strong> – {item.description}?</p><Error mutation={mutation} /></div><div className="modal-actions"><button className="button ghost" type="button" onClick={close}>Hủy</button><button className="button danger-button" type="button" disabled={mutation.isPending} onClick={() => mutation.mutate(item.id)}>{mutation.isPending ? "Đang xóa..." : "Xóa bảng giá"}</button></div></div></div>;
}
...
function Action({ icon, title, click, className = "", disabled = false }) { return <button className={`icon-button ${className}`} type="button" title={title} disabled={disabled} onClick={click}>{icon}</button>; }
